...
ProviderTuple = namedtuple('ProviderTuple', 'name title enabled provider_class')
...
def cover_art_providers():
    ...
    for p in sorted(ext_point_cover_art_providers, key=lambda p: (order[p.name].position, p.name)):
        yield ProviderTuple(name=p.name, title=p.display_title(), enabled=order[p.name].enabled, provider_class=p)
